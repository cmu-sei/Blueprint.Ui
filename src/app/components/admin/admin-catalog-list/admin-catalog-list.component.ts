...
export class AdminCatalogListComponent implements OnDestroy {
  ...
  displayedColumns: string[] = ['action', 'ispublic', 'name', 'injecttype', 'description'];
  injectTypeList: InjectType[] = [];
  private unsubscribe$ = new Subject();
  isExpansionDetailRow = (i: number, row: Object) => (row as Catalog).id === this.expandedElementId;
  expandedElementId = '';
  ...
  constructor(
    ...
    this.catalogQuery.selectAll().pipe(takeUntil(this.unsubscribe$)).subscribe(catalogs => {
  ...
  getSortedCatalogs(catalogs: Catalog[]) {
    if (catalogs) {
      catalogs.sort((a, b) => this.sortCatalogs(a, b, this.sort.active, this.sort.direction));
    }
    return catalogs;
  }
  ...
  copyCatalog(catalog: Catalog): void {
    // this.catalogDataService.copy(id);
  }

  sortChanged(sort: Sort) {
    this.sort = sort;
    this.catalogDataSource.data = this.getSortedCatalogs(this.getFilteredCatalogs(this.catalogList));
  }
  ...
  getFilteredCatalogs(catalogs: Catalog[]): Catalog[] {
    ...
    if (catalogs) {
      ...
      if (filteredCatalogs && filteredCatalogs.length > 0 && this.filterString) {
        const filterString = this.filterString?.toLowerCase();
        filteredCatalogs = filteredCatalogs.filter(catalog => catalog.name?.toLowerCase().includes(filterString));
      }
    }
